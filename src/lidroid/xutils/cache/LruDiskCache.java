...
package lidroid.xutils.cache;

import lidroid.xutils.util.IOUtils;
import lidroid.xutils.util.LogUtils;
...
public final class LruDiskCache implements Closeable {
	 ...
	 * 
	 * CLEAN 3400330d1dfc7f3f7f4b8d4d803dfcf6 832 21054 UPDATE
	 * 335c4c6028171cfddfbaae1a9c313c52 CLEAN 335c4c6028171cfddfbaae1a9c313c52
	 * 3934 2342 DELETE 335c4c6028171cfddfbaae1a9c313c52 UPDATE
	 ...
	 * and optional state-specific values. o UPDATE lines track that an entry is
	 * actively being created or updated. Every successful UPDATE action should
	 * be followed by a CLEAN or DELETE action. UPDATE lines without a matching
	 * CLEAN or DELETE indicate that temporary files may need to be deleted. o
	 ...
	 * values. o READ lines track accesses for LRU. o DELETE lines track entries
	...
	public static LruDiskCache open(File directory, int appVersion,
		...
		if (cache.journalFile.exists()) {
			try {
				...
				LogUtils.e("DiskLruCache " + directory + " is corrupt: "
		...
		try {
			...
			for (Entry entry : lruEntries.values()) {
				if (entry.currentEditor != null) {
					...
					writer.write(CLEAN + " " + entry.diskKey + " "
							+ EXPIRY_PREFIX + entry.expiryTimestamp
							+ entry.getLengths() + '\n');
				}
			}
		} finally {
	...
	private synchronized void completeEdit(Editor editor, boolean success)
		...
		if (entry.readable | success) {
			entry.readable = true;
			journalWriter.write(CLEAN + " " + entry.diskKey + " "
					+ EXPIRY_PREFIX + entry.expiryTimestamp
					+ entry.getLengths() + '\n');
			if (success) {
